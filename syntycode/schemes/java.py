...
def format_function(formatter, name, properties, children_ids, indent_level, indent):
    ...
    inputs = ", ".join([f"Object {inp}" for inp in raw_inputs])
    fn_name = translate_constructor_name(name, "__CLASS__")
    
    if fn_name == "__CLASS__":
        # Constructor: name will be set by format_class
        code = f"{indent}public void {name}({inputs}) {{\n"
    else:
        code = f"{indent}public void {fn_name}({inputs}) {{\n"
    if children_ids:
...
def format_variable(formatter, name, properties, children_ids, indent_level, indent):
    ...
    if not clean_name.startswith("this."):
        return f"{indent}Object {clean_name} = {val_code};"
...
def format_class(formatter, name, properties, children_ids, indent_level, indent):
    ...
    if children_ids:
        ...
        for cid in children_ids:
            ...
            # Replace __init__ with class name for Java constructors
            rendered = rendered.replace(f"public void __init__", f"public {name}")
