...
def format_variable(formatter, name, properties, children_ids, indent_level, indent):
    val_code = formatter.format_node(children_ids[0], 0) if children_ids else "inherit"
    # Mapping variable assignment to CSS variables
    return f"{indent}--{name}: {val_code};"

def format_call(formatter, name, properties, children_ids, indent_level, indent):
    args = " ".join(properties.get("args", []))
    # Mapping calls to CSS properties conceptually
    return f"{indent}{name}: {args};"

def format_condition(formatter, name, children_ids, indent_level, indent):
    code = ""
    if len(children_ids) >= 2:
        check_code = formatter.format_node(children_ids[0], 0).replace(';', '')
        code += f"{indent}@media ({check_code}) {{\n"
