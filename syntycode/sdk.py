...
class GraphContext:
    ...
    def _traverse_and_insert(self, cursor, node: Node, module_id: int, parent_id: Optional[int] = None, seq_index: int = 0) -> int:
        # Determine scope path (basic approximation for SDK)
        scope_path = node.name
        
        cursor.execute(
            "INSERT INTO ast_nodes (module_id, node_type, name, scope_path, properties) VALUES (?, ?, ?, ?, ?)",
            (module_id, node.node_type, node.name, scope_path, json.dumps(node.properties))
        )
        ...
        if parent_id is not None:
            cursor.execute(
                ...
                (parent_id, node_db_id, "CONTAINS", seq_index)
            )
            
        for i, child in enumerate(node.children):
            self._traverse_and_insert(cursor, child, module_id, node_db_id, i)
            
        # We handle calls/references simply (they'd need linking in a real compiler pass)
        return node_db_id
    ...
    def commit_to_db(self):
        ...
        # Insert module
        cursor.execute(
            "INSERT INTO modules (module_name) VALUES (?) ON CONFLICT(module_name) DO UPDATE SET last_updated=CURRENT_TIMESTAMP",
            (self.module.name,)
        )
        cursor.execute("SELECT module_id FROM modules WHERE module_name = ?", (self.module.name,))
        module_id = cursor.fetchone()[0]
        
        # Purge old module nodes (cascade handles edges)
        cursor.execute("DELETE FROM ast_nodes WHERE module_id = ?", (module_id,))
        
        # Insert graph recursively
        self._traverse_and_insert(cursor, self.module, module_id)
